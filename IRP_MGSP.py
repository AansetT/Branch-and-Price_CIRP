...
def SubProblem(ncustomers, psi, delta, phi, h, d, k, t, vehicleCapacity, DualPrices, DualZero, Gap_Limit = False):
    ...
    if Gap_Limit:
        ...
        model.setParam('TimeLimit', 15) ## Change to 20 for the other instance
    ...
    # Parameter tunning
    # model.setParam('TuneTimeLimit', 30*60)
    ...
    # x = model.addVars(x_indeces, name = 'x', vtype = GRB.CONTINUOUS, lb=0, ub=1)
    z = model.addVars(z_indeces, name = 'z', vtype = GRB.CONTINUOUS, lb=0)
    T = model.addVar(name = 'T', vtype = GRB.CONTINUOUS, lb=0, ub=1000)
    T_reciprocal = model.addVar(name = 'T_reciprocal', vtype = GRB.CONTINUOUS, lb=1/1000, ub=1/0.0001)
