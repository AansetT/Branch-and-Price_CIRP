...
def ColumnGeneration(MultiTours,Cost,T,X, instance_parameters: dict, constraints: dict = None, verbose = True):
    ...
    iteration = 0
    count = 0
    while RC_star < -1e-8 and iteration < 50:
        ...
        if verbose:
            print_iteration(iteration, RC_star, new_MultiTour, Min_TC, IPgap)
        # results.append([iteration, RC_star, new_MultiTour, T_new, Min_TC])
        iteration += 1
    return X, Min_TC, solutionRMP

def ColumnGeneration_V2(MultiTours,Cost,T,X, instance_parameters: dict, constraints: dict = None, verbose = True, Network = None, Time = None, Customers = None, heuristic = True):
    ncustomers, nvehicles, psi, delta, phi, h, d, k, t, vehicleCapacity = get_instance_parameters(instance_parameters)
    results = []
    RC_star = -1
    iteration = 0
    count = 0
    while RC_star < -1e-8 and iteration < 70:
        # Restrincted Master Problem
        RMP, w_var = IRP_LPref.MasterProblem(MultiTours, ncustomers, nvehicles, Cost, X, LPrelaxation=True)
        RMP = AddConstraints(MultiTours=MultiTours, constraints=constraints, MPmodel=RMP, w=w_var)
        Min_TC = IRP_LPref.MinTC_LPref(RMP)
        ...
        # Multi-tour Generation Subproblem
        if heuristic:
            Initial_Basic_Tours = MT.initial_basic_tours(ncustomers, Network)
            new_MultiTour, RC_star, T_new = IRP_MGSP_heuristic.SubProblem_Heuristic(Initial_Basic_Tours, ncustomers, psi, delta, phi, h, d, k, t, vehicleCapacity, Time, Customers, DualPrices, DualZero)
            IPgap = 1
            if min(RC_star) > -1e-8:
                return X, Min_TC, solutionRMP
        else:
            SP, x_var, T_var = IRP_MGSP.SubProblem(ncustomers, psi[0], delta, phi, h, d, k, t, vehicleCapacity[0], DualPrices, DualZero, Gap_Limit=True)
            SP = AddConstraints(MultiTours=MultiTours, ncustomers=ncustomers, constraints=constraints, SPmodel=SP, x=x_var)
            RC_star, IPgap = IRP_MGSP.MinRC(SP)
            if RC_star > -1e-8:
                return X, Min_TC, solutionRMP
            new_MultiTour, T_new = IRP_MGSP.SolutionSP(SP, x_var, T_var)
            new_MultiTour = [new_MultiTour]
            T_new = [T_new]
            RC_star = [RC_star]

        internal_count = 0
        for i, new_tour in enumerate(new_MultiTour):
            # Binary representation of the new MultiTour
            new_x = MT.Multitours_Binary([new_tour], ncustomers)

            # Calculate cost of the new MultiTour
            new_cost = MT.Multitour_Cost(1, ncustomers, psi, [T_new[i]], delta, t, new_x, phi, h, d)

            # Check if the new MultiTour is already in the MultiTours list
            idx = MT.MultiTour_index(MultiTours, new_tour)

            # Add the new MultiTour
            X, internal_count = Add_Column(MultiTours, Cost, T, X, new_tour, new_cost, T_new[i], new_x, idx, internal_count, RC_star)
        if internal_count == len(new_MultiTour):
            count += 1
        if count > 1:
            if verbose:
                print("No improvement in the last 1 iterations")
...
def ColumnGeneration_heuristic(MultiTours,Cost,T,X, instance_parameters: dict, Network = None, Time = None, Customers = None):
    ...
    # X, Min_TC_RMP, solutionRMP = ColumnGeneration_V2(MultiTours,Cost,T,X, instance_parameters, Network=Network, Time=Time, Customers=Customers)
...
def print_iteration(iteration, RC_star, new_MultiTour, Min_TC, MIPgap):
